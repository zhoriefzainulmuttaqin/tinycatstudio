...
@section('content')
    <section class="mx-auto max-w-7xl px-6 py-16 lg:px-8 lg:py-24">
        <div class="grid gap-8 lg:grid-cols-[1.1fr_0.9fr]">
            <div class="space-y-6">
                <span class="inline-flex rounded-full border border-orange-400/30 bg-orange-500/10 px-4 py-2 text-xs font-semibold tracking-[0.35em] text-orange-300">LAYANAN</span>
                <h1 class="text-4xl font-semibold text-white md:text-5xl">{{ $service->name }}</h1>
                <p class="max-w-3xl text-lg leading-8 text-white/65">{{ $service->description }}</p>
                <div class="flex flex-wrap gap-4">
                    <a href="#packages" class="inline-flex rounded-full bg-orange-500 px-6 py-3 text-sm font-semibold text-white transition hover:bg-orange-400">Lihat Paket</a>
                    <a href="{{ route('home') }}#consultation" class="inline-flex rounded-full border border-white/10 px-6 py-3 text-sm font-semibold text-white transition hover:border-orange-400/30 hover:bg-orange-500/10">Konsultasi Sekarang</a>
                </div>
            </div>
    ...
    <section id="packages" class="mx-auto max-w-7xl px-6 py-8 lg:px-8 lg:pb-16">
        ...
        <div class="grid gap-6 lg:grid-cols-3">
            @foreach ($service->pricingPackages as $package)
                <div class="rounded-[2rem] border {{ $package->is_popular ? 'border-orange-400/40 bg-orange-500/10' : 'border-white/10 bg-white/5' }} p-6">
        ...
        <div class="grid gap-6 lg:grid-cols-3">
            @forelse ($service->portfolios as $portfolio)
                <article class="overflow-hidden rounded-[2rem] border border-white/10 bg-white/5">
                    <img src="{{ $portfolio->thumbnail }}" alt="{{ $portfolio->title }}" class="h-56 w-full object-cover">
                    <div class="space-y-4 p-6">
                        ...
                        <a href="{{ route('portfolios.show', $portfolio) }}" class="inline-flex rounded-full border border-white/10 px-4 py-2 text-sm font-semibold text-white transition hover:border-orange-400/30 hover:bg-orange-500/10">Lihat Detail</a>
                    </div>
                </article>
            @empty
    ...
    <section class="mx-auto max-w-7xl px-6 py-8 lg:px-8 lg:pb-20">
        <div class="rounded-[2rem] border border-white/10 bg-white/5 p-8">
            <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                ...
                <a href="{{ route('home') }}#consultation" class="inline-flex rounded-full bg-orange-500 px-6 py-3 text-sm font-semibold text-white transition hover:bg-orange-400">Diskusikan Project</a>
            </div>
        </div>
    </section>
@endsection
