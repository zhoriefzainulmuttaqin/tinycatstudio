...
<html lang="id">
    ...
    <body class="min-h-screen bg-neutral-950 text-white antialiased">
        ...
        <div class="relative isolate overflow-hidden">
            ...
            <header x-data="{ open: false }" class="sticky top-0 z-40 border-b border-white/10 bg-neutral-950/85 backdrop-blur-xl">
                <div class="mx-auto flex max-w-7xl items-center justify-between px-6 py-4 lg:px-8">
                    ...
                    <nav class="hidden items-center gap-8 text-sm text-white/75 lg:flex">
                    ...
                    <button type="button" class="inline-flex h-11 w-11 items-center justify-center rounded-2xl border border-white/10 text-white lg:hidden" @click="open = !open" aria-label="Buka menu">
                        <span x-show="!open">Menu</span>
                        <span x-show="open" x-cloak>Tutup</span>
                    </button>
                </div>

                <div x-show="open" x-transition class="border-t border-white/10 px-6 py-5 lg:hidden">
                ...
                <div class="mx-auto grid max-w-7xl gap-10 px-6 py-16 lg:grid-cols-[1.4fr_1fr] lg:px-8">
                    <div class="space-y-5">
                        ...
                        <div class="flex flex-wrap gap-3 text-sm text-white/65">
                            ...
                            <span class="rounded-full border border-white/10 px-4 py-2">{{ $siteSettings['address'] ?? 'Indonesia' }}</span>
                        </div>
                    </div>

                    <form action="{{ route('contacts.store') }}" method="POST" class="space-y-4 rounded-[2rem] border border-white/10 bg-white/5 p-6">
                        @csrf
                        <div>
