...
<body>
    <div class="top-bar">
        <div>
            <h1 class="text-xl font-bold text-gray-900">Tagihan {{ $invoice->invoice_number }}</h1>
            <p class="text-sm text-gray-500">Dari: {{ $invoice->client->company_name ?? $invoice->client->name }}</p>
        </div>
    ...
    <div class="iframe-container">
        <object data="{{ route('invoices.public.stream', $invoice->invoice_number) }}" type="application/pdf" width="100%" height="100%">
            <p>Browser Anda tampaknya belum mendukung tampilan PDF langsung. <br>
               <a href="{{ route('invoices.public.download', $invoice->invoice_number) }}" class="text-indigo-600 underline">Klik di sini untuk mengunduh PDF</a>.
            </p>
        </object>
    </div>

    <div class="footer-banner">
        Ditenagai oleh <a href="https://tinycatstudio.tech" target="_blank" class="font-semibold text-indigo-600 hover:text-indigo-500 hover:underline">TinyCatStudio</a>
    </div>
</body>
</html>
